<template>
  <a-card :bordered="false">
          ...
          >
            ...
            <template v-slot:toolbar_buttons>
              <a-row class="toolbar_buttons_xgrid" :gutter="16">
                ...
                <a-col :lg="2" :md="2" :sm="2" align="left">
                  <a-button type="primary" @click="handleToggleSearch">
                    {{ searchFormConfig.toggleSearchStatus ? '精简' : '高级' }}
                    <a-icon :type="searchFormConfig.toggleSearchStatus ? 'down' : 'up'" />
                  </a-button>
                </a-col>
                <a-col :lg="12" :md="8" :sm="9" align="right">
                  <span class="table-page-search-submitButtons">
            ...
            <template slot-scope="{ row, column }" slot="recordAction">
              ...
              <a-tooltip placement="top" title="发布信息">
                <a-popconfirm title="确定发布本条消息吗？" @confirm="releaseById(row.id)" okText="确定" cancelText="取消">
                  <a-button type="danger" shape="circle" size="small">
                    <a-icon type="play-circle" theme="outlined"></a-icon>
                  </a-button>
                </a-popconfirm>
              </a-tooltip>
            </template>
    ...
    <a-modal
      ...
      :maskClosable="false"
    >
      <jhi-announcement-update @cancel="updateModalCancel" :showInModal="true" :updateEntityId="announcementId" />
    </a-modal>
  </a-card>
</template>
