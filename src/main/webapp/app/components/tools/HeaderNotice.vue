  ...
  methods: {
    ...
    loadData() {
      try {
        // 获取系统消息
        forkJoin([
          this.announcementService().retrieveUnread(AnnoCategory.NOTICE),
          this.announcementService().retrieveUnread(AnnoCategory.SYSTEM_INFO),
        ]).subscribe(
          ([noticeRes, infoRes]) => {
            this.announcement1 = noticeRes.data;
            this.msg1Count = noticeRes.data.length;
            this.msg1Title = '通知(' + this.msg1Count + ')';
            this.announcement2 = infoRes.data;
            this.msg2Count = infoRes.data.length;
